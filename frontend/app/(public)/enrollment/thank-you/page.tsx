...
function PaymentThankYouInner() {
  ...
  return (
    <PageShell>
      <PaymentStatusView enrollmentId={enrollmentId} t={t} />
    </PageShell>
  );
}
...
function PaymentStatusView({ enrollmentId, t }: PaymentStatusViewProps) {
  const { data, isLoading, isError } = useQuery<PaymentStatusDto>({
    ...
    queryFn: () => getPaymentStatusByEnrollment(enrollmentId, true),
