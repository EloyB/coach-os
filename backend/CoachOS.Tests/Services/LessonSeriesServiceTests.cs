...
[TestFixture]
public class LessonSeriesServiceTests
{
    ...
    private LessonSeriesService _service = null!;
    ...
    public void SetUp()
    {
        ...
        _service = new LessonSeriesService(
    ...
    private static LessonSeries BuildSeries(Guid? id = null, int enrollments = 0, int lessons = 0)
    {
    ...
    public async Task GetAllAsync_ReturnsEmptyList_WhenNoSeries()
    {
        _lessonSeriesRepo
            ...
            .ReturnsAsync(new List<LessonSeries>());
    ...
    [Test]
    public async Task GetAllAsync_ReturnsDtos_WithTrainerNamesAndLessonCounts()
    {
        LessonSeries series = BuildSeries();
        _lessonSeriesRepo
            .Setup(r => r.GetByOrganizationAsync(OrgId, null, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<LessonSeries> { series });
        ...
        LessonSeriesDto dto = result.Value![0];
        dto.TrainerName.Should().Be("Jan Jansen");
    ...
    {
        LessonSeries series = BuildSeries(lessons: 2);
        _lessonSeriesRepo
        ...
        _userLookup
            ...
            .ReturnsAsync("Jan Jansen");

        Result<LessonSeriesDto> result = await _service.GetByIdAsync(series.Id, OrgId);

        result.IsSuccess.Should().BeTrue();
        result.Value!.Id.Should().Be(series.Id);
        result.Value.TrainerName.Should().Be("Jan Jansen");
    ...
    public async Task GetByIdAsync_ReturnsNotFound_WhenMissing()
    {
        ...
        Result<LessonSeriesDto> result = await _service.GetByIdAsync(missingId, OrgId);
    ...
    public async Task GetMembersAsync_ReturnsMemberDtos()
    {
        ...
        Result<List<LessonSeriesMemberDto>> result = await _service.GetMembersAsync(OrgId);
    ...
    {
        CreateLessonSeriesRequest request = new()
        {
            TrainerId = TrainerId,
        ...
        _lessonSeriesRepo
            .Setup(r => r.AddAsync(It.IsAny<LessonSeries>(), It.IsAny<CancellationToken>()))
            .Callback<LessonSeries, CancellationToken>((s, _) => s.Id = Guid.NewGuid())
        ...
        _lessonSeriesRepo.Verify(
            r => r.AddAsync(It.Is<LessonSeries>(s => s.Name == "Zomerlessen 2026" && s.OrganizationId == OrgId),
    ...
    public async Task CreateAsync_ReturnsNotFound_WhenClubMissing()
    {
        ...
        result.IsSuccess.Should().BeFalse();
        result.Errors.Should().ContainSingle(e => e.Code == ErrorCodes.NotFound);
        _lessonSeriesRepo.Verify(r => r.AddAsync(It.IsAny<LessonSeries>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task UpdateAsync_ReturnsUpdatedDto_WhenValid()
    {
        ...
        UpdateLessonSeriesRequest request = new()
        {
            TrainerId = TrainerId,
        ...
        Result<LessonSeriesDto> result = await _service.UpdateAsync(series.Id, OrgId, request);
    ...
    public async Task UpdateAsync_ReturnsNotFound_WhenSeriesMissing()
    {
        ...
        Result<LessonSeriesDto> result = await _service.UpdateAsync(missingId, OrgId, request);
    ...
    {
        LessonSeries series = BuildSeries(enrollments: 0, lessons: 2);
        _lessonSeriesRepo
    ...
    {
        LessonSeries series = BuildSeries(enrollments: 3);
        _lessonSeriesRepo
            .Setup(r => r.GetByIdWithEnrollmentsAsync(series.Id, OrgId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(series);
        ...
        _lessonSeriesRepo.Verify(r => r.DeleteAsync(It.IsAny<LessonSeries>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task AddLessonAsync_ReturnsId_WhenValid()
    {
        ...
        result.Value.Should().NotBeEmpty();
        _lessonRepo.Verify(
            r => r.AddAsync(It.Is<Lesson>(l => l.CourtName == "Baan 3" && l.LessonSeriesId == series.Id),
                It.IsAny<CancellationToken>()),
            Times.Once);
    }
